...
class DiscreteMetaAction(ActionType):
    ...
    def act(self, action: int) -> None:
        self.controlled_vehicle.ast_action = action
...
class MultiAgentAction(ActionType):
    ...
    def space(self) -> spaces.Space:
        act_list = [len(action_type.actions) for action_type in self.agents_action_types]
        return spaces.MultiDiscrete(act_list)
