...
class MultiAgentObservation(ObservationType):
    ...
    def space(self) -> spaces.Space:
        obs_tmp = [{
            'vehicle' + str(i) : spaces.Box(shape=(self.agents_observation_types[i].vehicles_count, len(self.agents_observation_types[i].features)), low=-np.inf, high=np.inf, dtype=np.float32)
            } \
                    for i in range(len(self.agents_observation_types))]
        # return spaces.Tuple([obs_type.space() for obs_type in self.agents_observation_types])
        # convert obs_tmp to dict
        obs_tmp = dict((k, v) for d in obs_tmp for k, v in d.items())
        return spaces.Dict(obs_tmp)

    def observe(self, for_reward = False) -> tuple:
        if for_reward:
            return tuple(obs_type.observe() for obs_type in self.agents_observation_types)
        obs_tmp = [{
            'vehicle' + str(i) : self.agents_observation_types[i].observe()
            } for i in range(len(self.agents_observation_types))]
        obs_tmp = dict((k, v) for d in obs_tmp for k, v in d.items())
        # return tuple(obs_type.observe() for obs_type in self.agents_observation_types)
        return obs_tmp
