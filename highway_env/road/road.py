...
if TYPE_CHECKING:
    from highway_env.vehicle import kinematics, objects, controller
...
class Road(object):
    ...
    def neighbour_vehicles_ast(self, ego: 'kinematics.Vehicle', lane_index: LaneIndex = None) \
            -> Tuple[Optional['kinematics.Vehicle'], Optional['kinematics.Vehicle']]:
        """
        Find the preceding and following vehicles of a given vehicle.
        ...
        if not lane_index:
            return None, None
        lane = self.network.get_lane(lane_index)
        s = self.network.get_lane(lane_index).local_coordinates(ego.position)[0]
        s_front = s_rear = None
        v_front = v_rear = None
        # count number of vehicles in front of ego vehicle
        n_front = n_rear = 0
        for v in self.vehicles + self.objects:
            if v is not ego and not isinstance(v, Landmark):  # self.network.is_connected_road(v.lane_index,
                # lane_index, same_lane=True):
                s_v, lat_v = lane.local_coordinates(v.position)
                if not lane.on_lane(v.position, s_v, lat_v, margin=1):
                    continue
                if s <= s_v and (s_front is None or s_v <= s_front):
                    n_front += 1
                    if s_front is None or s_v < s_front:
                        s_front = s_v
                        v_front = v
                if s_v < s and (s_rear is None or s_v > s_rear):
                    n_rear += 1
                    s_rear = s_v
                    v_rear = v
        return v_front, v_rear, n_front, n_rear
